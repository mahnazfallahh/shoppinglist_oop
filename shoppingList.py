...
class DateTimeEncoder(JSONEncoder):
    ...
    def default(self, obj: int):
        """ override default method in datetime encoder in order to convert datetime # noqa E501
        value into Isoformat in order to serialize it which can write it in json file. # noqa E501
        Parameters
        ----------
        self
        obj
...
class Product():
    ...
    def use_discount(account_balance: int, code: str) -> int:
        """ method to calculate account balance which multiple it into 0.5 increase it and # noqa E501
        display to user
        Parameters
        ----------
        variable1 :
    ...
    @staticmethod
    def buy_categories(
        categories, category_bill: list[int],
        bill_name: str,
        account_balance: int
    ) -> int:
                ...
                for key, value in data.items():
                    if line == key:
                        ...
                        if bill > account_balance:
                            print("OOoops, sorry ! you don't have enough money to buy your list.", "\N{slightly frowning face}") # noqa E501
                            with open('database/history.json') as files:
                                user_history["account_balance"] = account_balance # noqa E501
                                user_history["is_buy"] = "you couldn't buy anything." # noqa E501
                                user_history["date_buy"] = datetime.datetime.now() # noqa E501
                                user_history["bill_name"] = bill_name
                                with open('database/history.json', 'w') as history: # noqa E501
                                    json.dump(user_history, history, indent=2, cls=DateTimeEncoder) # noqa E501
                        else:
                            balance: int = account_balance - bill
                            print("The purchase was made successfully!!", "\N{smiling face with smiling eyes}") # noqa E501
                            print(f"your account balance is : {balance}")
                            logger.info(f'user buy {bill_name} lists !!')
                            with open('database/history.json') as files:
                                user_history["account_balance"] = balance
                                user_history["is_buy"] = "you bought your list." # noqa E501
                                user_history["date_buy"] = datetime.datetime.now() # noqa E501
                                user_history["bill_name"] = bill_name
                                with open('database/history.json', 'w') as history: # noqa E501
                                    json.dump(user_history, history, indent=2, cls=DateTimeEncoder) # noqa E501
            ...
            with open('database/shopping-list.json') as groceries:
                ...
                for item in range(items_number):
                    ...
                    if items not in grocery:
                        print('OOops ! no such item is in shopping list.', "\N{slightly frowning face}") # noqa E501
                        print("Sorry ! we can't add it into the list .", "\N{slightly frowning face}") # noqa E501
                    else:
                        grocery_items[items] = number_items
                        products_number: int = len(grocery_items)
                        print(f'{products_number} item added successfully in grocery list!!!') # noqa E501
    ...
    def show_category(files):
        """ function for iterate into the json file to display items of categories. # noqa E501
        Parameters
        ----------
        var1 : files
        """
    ...
    def edit_grocery(edit_item: str, item_edit_with: str) -> str:
        ...
        variable2 :
            item_edit_with is specific item which user want to edit it with edit_item""" # noqa E501
        with open('database/shopping-list.json') as files:
